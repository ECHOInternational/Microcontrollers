...
// This is a C++ (cpp) file modified as explained here: https://www.youtube.com/watch?v=amkT5NtOgWc.
...
static uint8_t LoRaClass::sendSPItiny85(uint8_t spiData)
{
 ...
 while (((1 << USIOIF) & USISR) == 0 )  //transfer until the overlow bit is set
 {
  USICR = (1 << USIWM0) | (1 << USICS1) | (1 << USITC) | (1 << USICLK); // put tiny 85 into SPI mode | external clock 
 }
 return USIDR;  // retrieve data from Slave at end of 1 byte transfer
}
void LoRaClass::beginTinySPI(){
  DDRB |= (1 << spiSCK);  // set SCK as output - SPI clock
  DDRB |= (1 << spiDO);  // set DO as output  - set data out
...
uint8_t LoRaClass::singleTransfer(uint8_t address, uint8_t value)
{
  ...
  PORTB &= ~(1<<ssPIN);  //Pull as low
  sendSPItiny85(address);  // address to read/write to slave
  response = sendSPItiny85(value);  // retrieve byte from slave
